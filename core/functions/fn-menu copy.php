...
function canva_lang_selector_submenu()
{
	if (is_wpml_activated()) {
		...
		if (!empty($languages)) {
			...
			$html .= '<li class="menu-item menu-item-has-children menu-item-lang-' . $current_language[0]['lang_code'] . '">';
...
function canva_get_extra_menu_items($menu_item = '')
{
	...
	if ('bag-icon' == $menu_item) {
		...
		if (is_woocommerce_activated()) {
			...
			$html .= '<a class="bag-icon cursor-not-allowed" aria-label="' . __('Cart is empty', 'canva-frontend') . '">' . canva_get_svg_icon($name = 'fontawesome/' . $menu_fontawesome_icon_type . '/shopping-bag', $css_classes = 'fill-current') . '</a>';
		}
		$html .= '</li>';
	}
	...
	if ('cart-icon' == $menu_item) {
		...
		if (is_woocommerce_activated()) {
			...
			$html .= '<a class="cart-icon cursor-pointer" aria-label="' . __('Cart is empty', 'canva-frontend') . '">' . canva_get_svg_icon($name = 'fontawesome/' . $menu_fontawesome_icon_type . '/shopping-cart', $css_classes = 'fill-current') . '</a>';
		}
		$html .= '</li>';
	}
	...
	// $extra_items = 'ciao';
	// $html .= apply_filters('canva_extra_menu_items', $extra_items);
...
function canva_menu_builder()
{
	...
	$off_canvas_desktop_full_width = '';
	if (!$off_canvas_desktop['full_width']) {
		$off_canvas_desktop_full_width = 'container';
	}

	$off_canvas_mobile_full_width = '';
	if (!$off_canvas_mobile['full_width']) {
		$off_canvas_mobile_full_width = 'container';
	}
...
function canva_wp_nav_menu_items_icons($items, $args)
{
	foreach ($items as &$item) {
		...
		if ($icon) {
			...
			$html = '';
			$html .= '<div class="icon float-left">';
 ...
 * @return void
 */
function canva_menu_items_export()
{
	$location = '';
	...
	foreach ($menu_items as $menu_item) {
		...
		$items = ['menu_name' => $location, 'ID' => $menu_item->ID, 'parent' => $menu_item->menu_item_parent, 'url' => $menu_item->url, 'title' => $menu_item->title, 'menu_icon' => $menu_icon];
	}
...
add_action('wp_ajax_menu_items', 'canva_menu_items_export', 10, 10);
add_action('wp_ajax_nopriv_menu_items', 'canva_menu_items_export', 10, 1);
