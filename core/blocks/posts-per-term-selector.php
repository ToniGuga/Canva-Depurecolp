...
?>
	...
	if (!get_field('hide') || (get_field('hide') && get_field('show_for_admin') && current_user_can('edit_pages'))) {
		...
		$term_slug = str_replace([', ', ' '], ',', get_field('term_slug'));
		...
		$results_date_time_field_name = apply_filters('end_date_time_field_name', 'results_date_time');
		...
		if ($taxonomy_slug && $term_slug) {
			if ($coming && $during) {
				$args = [
					...
					'meta_type' => 'DATETIME',
					'tax_query' => [
						['taxonomy' => $taxonomy_slug, 'field' => 'slug', 'terms' => array_values($term_slug_array)],
					],
					'meta_query' => [
						//'relation' => 'AND',
				...
				$args = [
					...
					'meta_query' => [
						//array('key' => 'end_day_time','compare' => '>=','value' => $now,'type' => 'DATETIME')
