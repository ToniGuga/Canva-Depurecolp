...
<div id="<?php echo esc_attr($id); ?>" class="wp-block-photobutton">
		...
		<?php if ($link_wrap) : ?>
			...
			<div class="_photobutton _pb-collection _layer-wrap group <?php echo esc_attr($className); ?> <?php echo esc_attr($isdark); ?>">
				...
				<!-- Layer content -->
				<div class="_layer-content relative flex flex-col justify-end items-start p-8" style="min-height: var(--pb-min-height);">
					...
					<?php if ($photobutton_toptitle) : ?>
						<h3 class="_block font-secondary text-primary mr-4"><?php echo $photobutton_toptitle; ?></h3>
					<?php endif; ?>
					<?php if ($photobutton_title) : ?>
						<h3 class="_title h2 mb-0"><?php echo $photobutton_title; ?></h3>
					<?php endif; ?>
					<?php if ($photobutton_subtitle) : ?>
						<span class="block fs-h2 font-secondary uppercase mb-0"><?php echo $photobutton_subtitle; ?></span>
					<?php endif; ?>
					<?php if ($photobutton_text) : ?>
						<span class="block mt-4 max-w-48ch"><?php echo $photobutton_text; ?></span>
					<?php endif; ?>


					<?php if ($link_wrap) : ?>
						<span class="button hollow mt-8 mb-0 group-hover:bg-black <?php echo esc_attr($className) . ' ' . esc_attr($modal_class) . ' ' . esc_attr($button); ?>" onclick="ga('send', 'event', 'Action-Link', 'Photobutton', '<?php echo $track_title; ?>');" <?php echo $modal_data_contet; ?>><?php echo esc_html($button_title); ?></span>
					<?php else: ?>
						<a class="button hollow mt-8 mb-0 group-hover:bg-black <?php echo esc_attr($className) . ' ' . esc_attr($modal_class) . ' ' . esc_attr($button); ?>" <?php echo $href . ' ' . $target_link; ?> onclick="ga('send', 'event', 'Action-Link', 'Photobutton', '<?php echo $track_title; ?>');" <?php echo $modal_data_contet; ?>><?php echo esc_html($button_title); ?></a>
					<?php endif; ?>
