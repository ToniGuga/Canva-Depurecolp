...
<div id="<?php echo esc_attr($id); ?>" class="wp-block-photobutton">
		...
		<?php if ($link_wrap) : ?>
			...
			?>
			<div class="_photobutton _pb-fixed-ratio _pb-fixed-ratio-abac _layer-wrap group <?php echo esc_attr($class_name); ?> <?php echo esc_attr($isdark); ?>">
				...
				<div class="_layer-visual absolute">
					...
					if ($bg_image) {
						$bg_image = canva_get_img([
							...
							'thumb_size' => '960-free',
							'figure_class' => '_layer-bg _pb-bg transform-gpu group-hover:scale-110',
							'img_class' => '',
							'bg_content' => '',
							'caption' => 'off',
							'blazy' => 'on',
					...
					<?php else : ?>
						...
						<div class="_layer-bg _pb-bg b-lazy transform-gpu group-hover:scale-110" <?php echo $image . ' ' . $image_small; ?>></div>

					<?php endif; ?>

					<div class="_layer-filter _pb-filter"></div>
					<div class="_layer-graphics transform-gpu -skew-x-12 -translate-x-1/3"></div>
				...
				<!-- Layer content -->
				<div class="_layer-content absolute h-full w-2/3">
					...
					<div class="_content-wrap h-full p-6 flex flex-col justify-between">

						<div class="_content-section-1-wrap">
							<?php if ($icon) : ?>
								<?php
								echo canva_get_img([
									'img_id' => $icon,
							...
							<?php if ($photobutton_title) : ?>
								<div class="_title-wrap mb-6 line-clamp-3">
									<h3 class="_pb-title block lh-11 mb-0"><?php echo $photobutton_title; ?></h3>
								</div>
							<?php endif; ?>

							<!-- <?php if ($photobutton_subtitle) : ?>
								<span class="_pb-subtitle h6 mt-4 mb-0"><?php echo $photobutton_subtitle; ?></span>
							<?php endif; ?> -->

							<?php if ($photobutton_text) : ?>
								<span class="_pb-text block mt-4 fs-sm mb-0"><?php echo $photobutton_text; ?></span>
							<?php endif; ?>
						</div>


						<div class="_content-section-2-wrap">
							<?php if (!$hide_button) : ?>
								...
								<?php if (!$link_wrap) : ?>
									...
									<?php else: ?>
										<span class="hollow p-2 mb-0 fs-xs <?php echo esc_attr($button_classes) .' ' . esc_attr($modal_class); ?>" <?php echo $modal_data_contet; ?>><?php echo esc_html($button_title); ?></span>
									<?php endif; ?>
								</div>
							<?php endif; ?>
						</div>
